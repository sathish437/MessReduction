...
function PasswordInput({ value, onChange, placeholder = "Password", required = true, error = false }) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl border px-4 py-3 sm:py-3.5 transition-all w-full relative bg-[var(--color-surface)] ${error ? 'border-rose-500/50 bg-rose-500/5 focus-within:border-rose-400' : 'border-[var(--color-border)] focus-within:border-[var(--color-btn-primary)]'}`}>
