...
function StudentLogin({ onNavigate }) {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col font-sans bg-[var(--color-primary-bg)] text-[var(--color-text-primary)] selection:bg-teal-500/30 relative overflow-hidden">
            ...
            <header className="w-full flex items-center justify-between px-4 sm:px-8 py-5 border-b border-[var(--color-border)] bg-[var(--color-header)] text-[var(--color-text-primary)] sticky top-0 z-50 shadow-md">
        <div className="flex items-center gap-4">
          <img src={image} alt="GCES Logo" className="w-12 h-12 sm:w-16 sm:h-16 object-contain drop-shadow-md" />
          <div className="flex flex-col leading-tight">
            <span className="text-[10px] sm:text-xs font-bold tracking-widest text-[var(--color-text-primary)] uppercase mb-0.5">Government College of Engineering</span>
            <span className="text-lg sm:text-2xl font-bold tracking-tight">SRIRANGAM</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={toggleTheme} className="text-[var(--color-text-primary)] hover:text-[var(--color-text-primary)] p-2.5 bg-white/10 hover:bg-white/20 rounded-[10px] border border-[var(--color-border)] shadow-sm transition-all cursor-pointer">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12 z-10">
        <div className="w-full max-w-[420px]">
          <motion.div
            ...
            className="w-full rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)]  p-8 shadow-2xl relative overflow-hidden group"
          >
            ...
            <div className="relative z-10">
              <div className="mb-8">
                <p className="text-xs font-semibold tracking-[0.2em] text-[var(--color-btn-primary)] uppercase mb-2">Student Portal</p>
                <h2 className="text-2xl sm:text-3xl font-bold text-[var(--color-text-primary)] tracking-tight">{TITLE}</h2>
                <p className="text-sm sm:text-[15px] text-[var(--color-text-secondary)] mt-2 font-medium">Sign in with your student credentials</p>
              </div>

              <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                <div className="flex flex-col gap-1.5 w-full text-left">
                  <label htmlFor="identifier-input" className="text-sm font-semibold tracking-wide text-[var(--color-text-primary)] select-none">
                    Register Number / Roll Number
                  </label>
                  <div className={`flex items-center gap-3 rounded-xl border px-4 py-3.5 transition-all duration-300 relative group bg-[var(--color-primary-bg)] ${error ? 'border-rose-500/50 bg-rose-500/5 focus-within:border-rose-400' : 'border-[var(--color-border)] focus-within:border-[var(--color-btn-primary)] focus-within:bg-[var(--color-btn-primary)]/5 focus-within:ring-2 focus-within:ring-[var(--color-btn-primary)]/20'}`}>
                    <span className={`shrink-0 text-base transition-colors ${error ? 'text-rose-400' : 'text-[var(--color-text-secondary)] group-focus-within:text-[var(--color-btn-primary)]'}`}><FiUser size={18} /></span>
                    <input
                      id="identifier-input"
                      type="text"
                      ...
                      required
                      className="flex-1 bg-transparent focus:outline-none text-base text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)] font-medium w-full appearance-none"
                    />
                  </div>
                </div>
              ...
              <p className="text-center text-sm mt-6 text-[var(--color-text-secondary)] font-medium relative z-10">
                New student?{" "}
                <button
